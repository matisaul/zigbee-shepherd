        ...
        if (mode === 'soft' || mode === 1) {
            ...
            if (self._isZstack3) {
                ...
                steps = steps.concat([
                    ...
                    // TODO link key fix
                    // function () { return self.request('ZDO', 'getLinkKey', {ieeeaddr: '0x00124b001ca15b73'}).delay(1000000); },
                    // function () { return self.request('SYS', 'osalNvWrite', nvParams.securityMode).delay(100000); },
                    function () { return self.request('APP_CNF', 'bdbStartCommissioning', {mode: 0x04}).delay(10); },
            ...
            steps.concat([
                function () { return self.request('SYS', 'osalNvItemInit', self._isZstack3 ? nvParams.znpCfgItem3 : nvParams.znpCfgItem).delay(10).fail(function (err) {
                    return (err.message === 'rsp error: 9') ? null : Q.reject(err);  // Success, item created and initialized
                }); },
                function () { return self.request('SYS', 'osalNvWrite', self._isZstack3 ? nvParams.znpHasConfigured3 : nvParams.znpHasConfigured).delay(10); },
            ])
...
Controller.prototype.checkNvParams = function (callback) {
    ...
    steps = [
        function () { return self.request('SYS', 'osalNvRead', self._isZstack3 ? nvParams.znpHasConfigured3 : nvParams.znpHasConfigured).delay(10).then(function (rsp) {
