        ...
        if (mode === 'soft' || mode === 1) {
            ...
            if (self._isZstack3x0 || self._isZstack30x) {
                ...
                steps = steps.concat([
                    ...
                    function () { return self.request('APP_CNF', 'bdbSetChannel', {isPrimary: 0x1, channel: channel}).delay(3000); },
                    // function () { return self.request('APP_CNF', 'bdbSetChannel', {isPrimary: 0x0, channel: 0x0}).delay(10); },
                    function () { return self.request('UTIL', 'getDeviceInfo', {}).delay(10); },
...
Controller.prototype.checkNvParams = function (callback) {
    ...
    if(self._isZstack3x0 || self._isZstack30x) {
        steps.push(
            function () { return self.request('SYS', 'osalNvRead', nvParams.znpHasConfigured3).delay(10).then(function (rsp) {
